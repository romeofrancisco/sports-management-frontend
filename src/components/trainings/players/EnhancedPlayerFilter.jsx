...
/**
 * Enhanced visual filter component for players matching admin teams style
 */
const EnhancedPlayerFilter = ({ teams, filters, onFilterChange }) => {
  ...
  const hasActiveFilters = filters.search || filters.sport || filters.team;
  ...
  const getTeamName = () => {
    if (!filters.team || !teams) return null;
    const team = teams?.find((t) => t.slug === filters.team);
        ...
        <div className="flex flex-wrap lg:flex-nowrap items-center gap-2 lg:gap-3">
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <Select
              ...
              onValueChange={(value) => handleFilterChange("team", value === "all" ? null : value)}
            >
              <SelectTrigger className="min-w-[140px] lg:min-w-[160px] bg-transparent border-muted-foreground/20">
