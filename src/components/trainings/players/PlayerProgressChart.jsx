    ...
    <Line
        ...
        datasets: [
          {
            ...
            data: selectedMetricData.data_points.map(
              (point) => point.value || 0
            ),
            borderColor: "#8884d8",
      ...
      options={{
        ...
        plugins: {
          ...
          annotation:
            selectedMetricData.data_points.length > 1
              ? {
                  annotations: {
                    startingLine: {
                      type: "line",
...
const SelectMetricPrompt = () => (
  <div className="flex flex-col items-center justify-center py-10 text-center">
    <p className="text-muted-foreground">
      Please select a metric to view player progress.
    </p>
  </div>
);
...
const PlayerProgressChart = ({
  ...
  const selectedMetricData =
    playerData.metrics_data && Array.isArray(playerData.metrics_data)
      ? playerData.metrics_data.find(
          (m) => String(m.metric_id) === String(selectedMetric)
        )
      : null;
...
const PerformanceAnalysis = ({ metricData }) => {
  if (
    !metricData ||
    !Array.isArray(metricData.data_points) ||
    metricData.data_points.length < 2
  ) {
  ...
  try {
    ...
    const lastPoint = metricData.data_points[
      metricData.data_points.length - 1
    ] || { value: 0 };
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              >
                {isImproved ? "Improvement" : "Decline"} from first to last
                record
              </p>
            </div>
          </CardContent>
          ...
          <CardContent className="pt-6">
            ...
            <ul className="space-y-1 text-sm">
              ...
              <li>
                ...
                <span
                  className={isImproved ? "text-green-600" : "text-red-600"}
                >
