...
const MessagesList = ({
  ...
  }, [messages]);  // Function to scroll to bottom smoothly
  ...
  useEffect(() => {
    ...
    if (hasNewMessages && scrolledToBottomInitially.current) {
      ...
      const isReallyNewMessage =
        currentLastMessage && currentLastMessage.id !== lastMessageId.current;
  ...
  useEffect(() => {
    ...
    <ScrollArea className="h-full relative" ref={scrollRef}>
      <div className="p-4 space-y-4">
        {/* Loading indicator for fetching older messages */}
        {isFetchingNextPage && (
          <div className="flex absolute top-0 inset-x-0 justify-center py-2 z-10">
