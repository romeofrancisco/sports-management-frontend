...
import { useTrainingMetrics, useTrainingCategories, useAssignPlayerTrainingMetrics } from '@/hooks/useTrainings';
import { Loader2, Filter, Check, User } from 'lucide-react';
...
const PlayerMetricsConfigModal = ({ 
  ...
  const filteredMetrics = React.useMemo(() => {
    ...
    // Filter further based on the active tab
    if (activeTab === 'selected') {
      filtered = filtered.filter(metric => selectedMetrics.includes(metric.id));
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Select which metrics should be tracked for this player's training.
            {playerTraining.session_title && (
              <span className="block mt-1 text-sm">Session: {playerTraining.session_title} ({playerTraining.session_date})</span>
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col space-y-4 flex-1 overflow-hidden">
          ...
          <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
            <TabsList className="grid grid-cols-3">
              ...
              <TabsTrigger value="selected">Selected ({selectedMetrics.length})</TabsTrigger>
              <TabsTrigger value="unselected">Unselected ({allMetrics ? allMetrics.length - selectedMetrics.length : 0})</TabsTrigger>
            </TabsList>

            <TabsContent value={activeTab} className="flex-1 data-[state=active]:flex data-[state=active]:flex-col">
              {metricsLoading ? (
