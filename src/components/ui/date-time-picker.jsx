...
function Calendar({
  ...
  const MONTHS = React.useMemo(() => {
    let locale = enUS;
    if (
      props.locale?.options &&
      props.locale?.localize &&
      props.locale?.formatLong
    ) {
      locale = {
        options: props.locale.options,
        localize: props.locale.localize,
        formatLong: props.locale.formatLong,
      };
    }
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        month_caption: "flex justify-between items-center w-full px-4",
        caption_label: "text-sm font-medium",
        nav: "space-x-1 flex items-center ",
        button_previous: cn(
          buttonVariants({ variant: "outline" }),
          "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 absolute left-4 top-9 md:top-5",
          disableLeftNavigation() && "pointer-events-none"
        ),
        button_next: cn(
          buttonVariants({ variant: "outline" }),
          "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 absolute right-4 top-9 md:top-5",
        ...
        selected:
          "bg-primary text-white hover:bg-primary hover:text-primary-foreground focus:bg-primary focus:text-primary-foreground rounded-l-md rounded-r-md",
