    ...
    () => (
      ...
      >
        <DroppableArea date={date} className="w-full h-full py-2">
          ...
          >
            {cellEvents.length === 0 && !isMobile && !isPlayer() ? (
              <div className="w-full h-full flex justify-center items-center group">
                {
                  // Use injected Add/Edit dialog if provided by CalendarProvider (e.g. FacilityAddEditReservationDialog),
                  // otherwise fall back to the default AddEditEventDialog used by the generic calendar.
                  (() => {
                    const Dialog = AddEditDialog || AddEditEventDialog;
                    return (
