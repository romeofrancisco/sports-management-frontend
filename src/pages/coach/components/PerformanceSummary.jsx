...
const PerformanceSummary = ({ playerProgress }) => {
  ...
  ).length;  const performanceMetrics = [
    {
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {performanceMetrics.map((metric, index) => (
            ...
            >
              ...
              <div className={`absolute inset-0 bg-gradient-to-br ${metric.gradient} opacity-8 group-hover:opacity-12 transition-opacity duration-300`}></div>
              ...
              <div className="relative z-10 space-y-3">
                <div className={`mx-auto w-12 h-12 rounded-xl ${metric.iconBg} flex items-center justify-center shadow-md`}>
                  {metric.icon}
                </div>
                <div className={`text-3xl font-bold ${metric.color} drop-shadow-sm`}>
                  {metric.value}
                </div>
        ...
        <div className="mt-6 p-4 bg-gradient-to-r from-primary/5 to-secondary/5 rounded-lg border border-primary/20">
          <p className="text-sm text-muted-foreground text-center">
            <span className="font-bold text-primary">
              {Math.round((improvingCount / playerProgress.player_progress.length) * 100)}%
            </span> of your players are showing improvement
          </p>
        </div>
      </CardContent>
    </Card>
