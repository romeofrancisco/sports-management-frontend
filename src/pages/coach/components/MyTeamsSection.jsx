    ...
    <Card className="bg-card shadow-lg border-2 border-primary/20 hover:shadow-xl transition-all duration-300">
      ...
      <CardContent>
        {overview?.team_attendance?.length > 0 ? (
          <div className="space-y-3">            {overview.team_attendance.map((team, index) => (
              <div
                key={team.team_id || index}
                className="relative overflow-hidden border-2 border-primary/20 rounded-xl p-4 bg-gradient-to-r from-primary/5 to-secondary/5 transition-all duration-300 hover:shadow-lg hover:scale-[1.02] group"
              >
                {/* Enhanced background effects */}
                <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-secondary/10 to-transparent rounded-full blur-2xl opacity-60"></div>
                ...
                <div className="grid grid-cols-2 gap-3">                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
