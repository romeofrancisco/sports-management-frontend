...
import { Trophy, TrendingUp, BarChart2, Calendar } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

const TeamPerformanceStats = ({ comprehensiveStats, statistics }) => {
  if (!comprehensiveStats || !comprehensiveStats.top_teams || comprehensiveStats.top_teams.length === 0) {
    return null;
  }

  const topTeams = comprehensiveStats.top_teams;
  const activeSeasonsList = statistics?.active_seasons || 0;
  ...
  const performanceData = [
    {
      ...
      description: `${Math.round(
        (topTeams[0]?.win_percentage || 0) * 100
      )}% wins`,
    ...
    {
      title: "Most Games",
      value: topTeams.reduce(
        (max, team) => Math.max(max, team.games_played || 0),
        0
      ),
      icon: BarChart2,
    ...
    {
      title: "League Status",
      value: activeSeasonsList > 0 ? "Active" : "Inactive",
      icon: Calendar,
      description: `${activeSeasonsList} active season${
        activeSeasonsList !== 1 ? "s" : ""
      }`,
