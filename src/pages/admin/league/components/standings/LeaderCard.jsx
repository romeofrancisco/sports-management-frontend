...
export const LeaderCard = ({ player, stats, rank, isLeader = false }) => {
  const { statCodes, mainStat, additionalStats, icon } = useLeaderData(player, stats, rank);

  if (isLeader) {
    // Special leader display for #1 player
    return <TopLeaderCard player={player} stats={stats} mainStat={mainStat} additionalStats={additionalStats} icon={icon} />;
  }
        ...
        {additionalStats && (
          <div className="bg-gradient-to-r from-secondary/5 to-primary/5 rounded-lg p-4 border border-secondary/20 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h5 className="text-sm font-semibold text-foreground">Performance Overview</h5>
            </div>
            <div className="grid grid-cols-2 gap-2">
              {additionalStats.split(' | ').map((stat, index) => {
                const [name, value] = stat.split(': ');
                return (
                  <div key={index} className="text-center p-2 bg-background/50 rounded border border-border/30">
