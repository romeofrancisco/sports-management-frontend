/**
 * Chart utility functions for tournament data
 * Adapted from season charts utilities
 */
 ...
 * Converts standings data structure to performance data structure
 * @param {Array} teamPerformance - Raw team performance data (standings)
...
export const sanitizeTeamPerformance = (teamPerformance) => {  
  return teamPerformance?.map(team => {
    // Calculate averages from totals if they don't exist
    const matchesPlayed = team.matches_played || team.total_games || 0;
    
    // For points-based sports
    const pointsPerGame = team.points_per_game || 0;
    const pointsConcededPerGame = team.points_conceded_per_game || 0;
    
    // For set-based sports
    const pointsPerSet = team.points_per_set || 0;
    const pointsConcededPerSet = team.points_conceded_per_set || 0;
    
    // Use the appropriate scoring metrics
    const totalScored = team.points_scored || team.total_points_scored || (pointsPerGame * matchesPlayed) || 0;
    const totalConceded = team.points_conceded || team.total_points_conceded || (pointsConcededPerGame * matchesPlayed) || 0;
    
    // Calculate streak from wins if not provided (simple heuristic)
    const wins = team.wins || 0;
    const streak = team.max_streak || team.current_streak || wins; // Use wins as a fallback
    
    return {
      ...team,
      team_name: team.team_name || 'Unknown Team',
      // Support both avg_ and calculated values
      avg_points_scored: team.avg_points_scored || pointsPerGame || (matchesPlayed > 0 ? totalScored / matchesPlayed : 0),
      avg_points_conceded: team.avg_points_conceded || pointsConcededPerGame || (matchesPlayed > 0 ? totalConceded / matchesPlayed : 0),
      // Support various streak field names
      max_streak: streak,
      // Support both field names
      total_games: matchesPlayed,
      matches_played: matchesPlayed,
      // Include totals for set-based calculations
      total_points_scored: totalScored,
      total_points_conceded: totalConceded,
      sets_played: team.sets_played || 0,
  ...
  const winPercentageTeams = [...teamsWithGames]
    .map(team => {
      ...
      if (isSetsScoring) {
        ...
        const gamesWon = team.wins || team.matches_won || 0;
        const gamesPlayed = team.total_games || team.matches_played || 0;
...
/**
 * Prepare all chart data for tournaments in one function call
 * @param {Array} teamPerformance - Raw team performance data
 * @param {Boolean} isSetsScoring - Whether the sport uses sets scoring
 * @returns {Object} - Object containing all chart data
 */
export const prepareTournamentChartData = (teamPerformance, isSetsScoring) => {
  if (!teamPerformance) {
    return {
